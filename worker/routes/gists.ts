...
const GIST_ID_REGEX = /(?:https?:\/\/gist\.github\.com\/[^/]+\/)?([a-f0-9]+)/;
const SESSION_COOKIE_REGEX = /__session=([^;]+)/;
const EDIT_COOKIE_REGEX = /gp_edit_cap=([^;]+)/;
...
gistRoutes.post("/:gist_id/import", authMiddleware, async (c) => {
  ...
  const gistIdMatch = body.url.match(GIST_ID_REGEX);
  ...
  if (!ghResponse.ok) {
    ...
    return c.json(
      { error: `Failed to fetch gist (${ghResponse.status})` },
      502
    );
  }
  ...
  console.log(
    `DO initialize response status: ${initResponse.status}, body: ${await initResponse.text()}`
  );
...
gistRoutes.get("/:gist_id", async (c) => {
  ...
  console.log(
    `Meta response status: ${metaResponse.status}, body: ${metaText}`
  );
...
gistRoutes.get("/:gist_id/can-edit", async (c) => {
  ...
  const editCookieMatch = cookies.match(EDIT_COOKIE_REGEX);
  if (editCookieMatch) {
